...
const sql = read("supabase/migrations/202607130001_leather.sql") + "\n" + read("supabase/migrations/202607140002_discussions_lucky_draw.sql"), ignore = read(".gitignore"), workflow = read(".github/workflows/deploy.yml");
...
const dynamic = new Set(["articleCommentContent","articleCommentForm","articleCommentList","cancelDiscussionReply","deleteSectionCheck","deleteTaskCheck","editSectionColor","editSectionName","levelColor","levelName","levelNote","newPageLang","newPageName","newSectionColor","newSectionName","restoreVersionBtn","taskDesc","taskDue","taskTitle","turnstileScript"]);
...
assert.match(app, /protectedPages = new Set\(\["vault", "stress", "roadmap", "blogs", "blog-editor", "checkin", "admin"\]\)/);
